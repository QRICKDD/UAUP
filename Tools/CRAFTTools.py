...
def load_CRAFTmodel(device,feamap=False):
    #os.environ["CUDA_VISIBLE_DEVICES"] = "1"
    #加载模型
    CRAFT_path=GConfig.CRAFT_model_path
    craft_net = CRAFT(feamap=feamap)
...
def get_CRAFT_pred(model,img:torch.Tensor,square_size=1280,
                   device=GConfig.CRAFT_device,is_eval=False,feamapLoss = False):
    ...
    score_link = y[0, :, :, 1]
    if feamapLoss:
        return score_text, score_link, target_ratio,_
